...
def verify(ep, dim, shape, nobjmax, npif, izap):
    ...
    def initialize_arrays():
        return np.ones((nobjmax + 1, shape[0], shape[1])) * npif, np.ones((nobjmax + 1, shape[0], shape[1])) * npif

    if dim == 'x':
        nxi_pif, nxf_pif = initialize_arrays()
            ...
            for j in range(shape[1]):
                ...
                for i in range(1, shape[0]):
                    if not ep[i, j, k]:
                        ...
                        inum += 1
                        if inum == 0 and iflu - izap < npif:
                            nxi_pif[inum, j, k] = iflu - izap
                            ising += 1
                        elif iflu - izap < npif:
                            nxi_pif[inum, j, k] = iflu - izap
                            nxf_pif[inum - 1, j, k] = iflu - izap
                            ising += 1
                        iflu = 0
                    if ep[i, j, k]:
                        iflu = 0

                if not ep[shape[0] - 1, j, k] and iflu - izap < npif:
                    nxf_pif[inum, j, k] = iflu - izap
                    ising += 1

        return nxi_pif, nxf_pif

    elif dim == 'y':
        nyi_pif, nyf_pif = initialize_arrays()
            ...
            for i in range(shape[0]):
                ...
                for j in range(1, shape[1]):
                    if not ep[i, j, k]:
                        ...
                        inum += 1
                        if inum == 0 and iflu - izap < npif:
                            nyi_pif[inum, i, k] = iflu - izap
                            ising += 1
                        elif iflu - izap < npif:
                            nyi_pif[inum, i, k] = iflu - izap
                            nyf_pif[inum - 1, i, k] = iflu - izap
                            ising += 1
                        iflu = 0
                    if ep[i, j, k]:
                        iflu = 0

                if not ep[i, shape[1] - 1, k] and iflu - izap < npif:
                    nyf_pif[inum, i, k] = iflu - izap
                    ising += 1

        return nyi_pif, nyf_pif

    elif dim == 'z':
        nzi_pif, nzf_pif = initialize_arrays()
            ...
            for i in range(shape[0]):
                ...
                for k in range(1, shape[2]):
                    if not ep[i, j, k]:
                        ...
                        inum += 1
                        if inum == 0 and iflu - izap < npif:
                            nzi_pif[inum, i, j] = iflu - izap
                            ising += 1
                        elif iflu - izap < npif:
                            nzi_pif[inum, i, j] = iflu - izap
                            nzf_pif[inum - 1, i, j] = iflu - izap
                            ising += 1
                        iflu = 0
                    if ep[i, j, k]:
                        iflu = 0

                if not ep[i, j, shape[2] - 1] and iflu - izap < npif:
                    nzf_pif[inum, i, j] = iflu - izap
                    ising += 1

        return nzi_pif, nzf_pif
